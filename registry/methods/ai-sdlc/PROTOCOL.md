...
|---|---|---|---|
| readiness | product-owner | `readiness-assessment` artifact; clarifications resolved | Gate `readiness-approved` passes, or a clarification remains open |
| intent | product-owner, architect | `intent` artifact; criteria elaborated | Gate `intent-framed` passes, or the Product Owner withholds approval |
| inception | architect | `requirements`, `architecture`; criteria designed | Gate `architecture-approved` passes, or a requirements gap sends work back to `intent` |
